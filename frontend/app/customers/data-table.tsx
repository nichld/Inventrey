...
export function DataTable() {
  ...
  const handleAddCustomer = async () => {
    if (newCustomerName.trim()) {
      try {
        const response = await axios.post("http://backend:5000/api/customers", { fullName: newCustomerName })
  ...
  const handleUpdateCustomer = async () => {
    if (editingCustomer) {
      try {
        const response = await axios.put(`http://backend:5000/api/customers/${editingCustomer._id}`, {
  ...
  const handleDeleteCustomer = async (id: string) => {
    try {
      await axios.delete(`http://backend:5000/api/customers/${id}`)
